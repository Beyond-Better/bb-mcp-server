...
// Import types
import type { ToolRegistry, CoreToolsDependencies, CreateMessageRequest, CreateMessageResult, ElicitInputRequest, ElicitInputResult } from '../types/BeyondMcpTypes.ts';
...
export class CoreTools {
  ...
  private registerServerStatusTool(registry: ToolRegistry): void {
    registry.registerTool('get_server_status', {
  ...
  private registerTestSamplingTool(registry: ToolRegistry): void {
    registry.registerTool('test_sampling', {
      title: 'Test Sampling',
      description: 'Test sampling tool that makes a request to the MCP client for text generation',
      category: 'core',
    ...
    }, async (args, extra) => this.handleTestSampling(args, extra as Record<string, unknown> | undefined));
  }
  ...
  private registerTestElicitationTool(registry: ToolRegistry): void {
    registry.registerTool('test_elicitation', {
      title: 'Test Elicitation',
      description: 'Test elicitation tool that makes a request to the MCP client for user input/approval',
      category: 'core',
      tags: ['testing', 'mcp-api', 'elicitation'],
      inputSchema: {
        message: z.string().describe('Message to present to the user'),
        requestedSchema: z.string().describe('Requested schema to use for elicitation (JSON string)'),
      },
    }, async (args, extra) => this.handleTestElicitation(args, extra as Record<string, unknown> | undefined));
  }
  ...
  private async handleTestSampling(
    ...
    try {
      ...
      const samplingResult = await this.sdkMcpServer.server.createMessage({
        _meta: (extra?._meta || {}) as { [x: string]: unknown; progressToken?: string | number | undefined },
        model: model,
        messages: [{
          role: 'user',
