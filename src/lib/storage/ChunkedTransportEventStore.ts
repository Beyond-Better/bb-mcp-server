...
export class TransportEventStoreChunked extends TransportEventStore {
  ...
  private async storeMessageInChunks(
    ...
    const chunkCount = Math.ceil(dataSize / this.config.maxChunkSize);

    // Break data into chunks
    for (let i = 0; i < chunkCount; i++) {
      const start = i * this.config.maxChunkSize;
      const end = Math.min(start + this.config.maxChunkSize, dataToStore.length);
      const chunkData = dataToStore.slice(start, end);
    ...
    this.logger?.debug('ChunkedTransportEventStore: Stored chunked event', {
      streamId,
      eventId,
      chunkCount,
      compressed,
  ...
  private async reassembleMessage(eventId: string, streamId: string): Promise<JSONRPCMessage | null> {
    try {
      ...
      this.logger?.error('ChunkedTransportEventStore: Failed to reassemble message', toError(error), {
        eventId,
        streamId,
      });
      return null;
  ...
  async storeEvent(streamId: string, message: JSONRPCMessage): Promise<string> {
    ...
    try {
      ...
      try {
        ...
        this.logger?.warn('ChunkedTransportEventStore: Failed to update stream metadata', {
          streamId,
          eventId,
          error,
        });
      ...
      this.logger?.error('ChunkedTransportEventStore: Failed to store event', toError(error), {
        streamId,
      });
      throw error;
    }
  ...
  async replayEventsAfter(
    ...
    if (!streamId) {
      this.logger?.warn('ChunkedTransportEventStore: Invalid lastEventId format', { lastEventId });
      return '';
    }
    ...
    try {
        ...
        for (let i = lastEventIndex + 1; i < events.length; i++) {
          ...
          if (eventEntry) {
            ...
            if (message) {
              ...
              this.logger?.warn('ChunkedTransportEventStore: Failed to reassemble message for replay', {
                eventId,
                streamId,
              });
            }
      ...
      this.logger?.info('ChunkedTransportEventStore: Replayed events', {
        streamId,
        lastEventId,
        replayedCount,
      });
      ...
      this.logger?.error('ChunkedTransportEventStore: Failed to replay events', toError(error), {
        streamId,
        lastEventId,
      });
      return '';
    ...
    try {
      ...
      for (let i = 0; i < toDelete.length; i += batchSize) {
        ...
        try {
          ...
          this.logger?.warn('ChunkedTransportEventStore: Failed to delete batch of old events', {
            streamId,
            batchStart: i,
            error: toError(error),
          });
      ...
      this.logger?.info('ChunkedTransportEventStore: Cleaned up old events', {
        streamId,
        deletedCount,
      ...
      this.logger?.error('ChunkedTransportEventStore: Failed to cleanup old events', toError(error), {
        streamId,
      });
      return 0;
    }
  ...
  async getChunkStatistics(streamId?: string): Promise<{
    ...
    try {
      ...
      this.logger?.error('ChunkedTransportEventStore: Failed to get chunk statistics', toError(error));
      return {
        totalEvents: 0,
        totalChunks: 0,
        averageChunksPerEvent: 0,
