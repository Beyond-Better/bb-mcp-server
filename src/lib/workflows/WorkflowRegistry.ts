...
export class WorkflowRegistry {
  ...
  private validateRegistration(registration: WorkflowRegistration): string[] {
    ...
    if (!this.validCategories.has(registration.category)) {
      ...
      errors.push(`Invalid workflow category: ${registration.category}. Available categories: ${availableCategories}`)
    }
  ...
  addValidCategory(category: string): void {
    if (!this.config.allowDynamicCategories) {
      throw new Error('Dynamic category registration is disabled. Set allowDynamicCategories: true in config.')
    }
