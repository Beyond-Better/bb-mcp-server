...
import { getAllDependenciesAsync } from './DependencyHelpers.ts';
...
export class AppServer {
   ...
   */
  static async create(dependencies?: Partial<AppServerDependencies>): Promise<AppServer> {
    const resolvedDependencies = await getAllDependenciesAsync(dependencies || {});
  ...
  async start(): Promise<void> {
    if (!this.initialized) {
      throw new Error('AppServer not initialized - use AppServer.create() instead of new AppServer()');
    }
