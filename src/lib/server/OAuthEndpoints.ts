...
export class OAuthEndpoints {
  ...
  private async handleAuthorize(request: Request): Promise<Response> {
    ...
    try {
      ...
      const authResponse = await this.oauthProvider.handleAuthorizeRequest(authRequest_clean, userId);
  ...
  private async handleToken(request: Request): Promise<Response> {
    ...
    try {
      ...
      // Validate grant_type
      if (tokenRequest.grant_type !== 'authorization_code' && tokenRequest.grant_type !== 'refresh_token') {
        return this.generateOAuthError('unsupported_grant_type', `Grant type '${tokenRequest.grant_type}' is not supported`);
      }
  ...
  private async handleCallback(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (error) {
        ...
        this.logger.error(`OAuthEndpoints: OAuth callback error [${callbackId}]:`, toError(errorMsg));
