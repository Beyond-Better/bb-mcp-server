...
import type { Transport, TransportConfig, TransportType, TransportDependencies, MCPRequest, MCPResponse, BeyondMcpAuthContext, TransportMetrics, SessionData } from './TransportTypes.ts';
...
export class TransportManager {
  ...
  async initialize(sdkMcpServer: SdkMcpServer): Promise<void> {
    ...
    try {
      ...
      await this.dependencies.eventStore.logEvent({
        ...
        transport: this.config.type,
        level: 'error',
        message: `Failed to initialize transport manager: ${error instanceof Error ? error.message : 'Unknown error'}`,
  ...
  async start(): Promise<void> {
    if (!this.sdkMcpServer) {
      throw new Error('Cannot start TransportManager - SDK MCP server not set. Call initialize() first.');
    }
  ...
  async handleHttpRequest(request: Request, authContext?: BeyondMcpAuthContext): Promise<Response> {
    ...
    try {
      const response = await this.httpTransport.handleHttpRequest(request, this.sdkMcpServer, authContext);
   ...
   */
  async switchTransport(newType: TransportType, newConfig?: Partial<TransportConfig>): Promise<void> {
    if (!this.initialized) {
   ...
   */
  async createSession(sessionData: Omit<SessionData, 'id' | 'createdAt' | 'lastActiveAt' | 'expiresAt' | 'transportType'>): Promise<string> {
    if (this.config.type !== 'http' || !this.httpTransport) {
