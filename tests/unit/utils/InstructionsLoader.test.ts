...
import { loadInstructions, validateInstructions, getInstructionsLoadingSummary } from '../../../src/lib/utils/InstructionsLoader.ts';
...
Deno.test('InstructionsLoader', async (t) => {
  ...
  await t.step('should load instructions from default file when no config or file path', async () => {
...
Deno.test('validateInstructions', async (t) => {
  ...
  await t.step('should reject instructions without key content', () => {
    const invalidInstructions = 'This is a long text that does not mention the required terms. It has sufficient length but lacks the necessary content indicators that would make it valid for our purposes.';
