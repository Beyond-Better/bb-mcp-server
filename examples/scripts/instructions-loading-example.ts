...
import {
  ...
  type InstructionsLoadingSummary,
  loadInstructions,
...
async function demonstrateValidation() {
  ...
  const testCases = [
    {
      name: 'Valid workflow instructions',
      content:
        'MCP Server with workflow capabilities. This is a comprehensive set of instructions.',
    },
    { name: 'Too short', content: 'Short' },
    {
      name: 'No MCP or workflow content',
      content:
        'This is a long text that does not mention the key terms we are looking for. It has enough length but lacks the required content indicators that make it valid.',
    },
    {
      name: 'Has MCP Server',
      content:
        'This MCP Server provides comprehensive functionality with detailed instructions and examples for users.',
    },
    {
      name: 'Has workflow',
      content:
        'This system provides workflow automation capabilities with comprehensive instructions and validation.',
    },
  ];
  ...
  for (const testCase of testCases) {
    ...
    console.log(
      `${testCase.name}: ${isValid ? '✅ Valid' : '❌ Invalid'} (${testCase.content.length} chars)`,
    );
  }
