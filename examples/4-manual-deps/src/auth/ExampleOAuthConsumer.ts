...
export class ExampleOAuthConsumer extends OAuthConsumer {
   ...
   */
  protected override async exchangeCodeForTokens(code: string, codeVerifier?: string): Promise<TokenResult> {
    try {
      ...
      this.logger?.error('ExampleCorp token exchange failed', error instanceof Error ? error : new Error(String(error)))
      return {
        success: false,
  ...
  protected override async refreshTokens(refreshToken: string): Promise<TokenResult> {
    try {
      ...
      this.logger?.error('ExampleCorp token refresh failed', error instanceof Error ? error : new Error(String(error)))
      return {
        success: false,
  ...
  override async getAccessToken(userId: string): Promise<string | null> {
    try {
      ...
      this.logger?.error('ExampleCorp access token retrieval failed', error instanceof Error ? error : new Error(String(error)), {
        userId,
  ...
  override async initialize(): Promise<void> {
    try {
      ...
      this.logger?.error('ExampleCorp OAuth consumer initialization failed', error instanceof Error ? error : new Error(String(error)))
      throw error
    }
  }
   ...
   */
  private async storeExampleCorpMetadata(userId: string, metadata: Record<string, any>): Promise<void> {
  ...
  private async verifyExampleCorpEndpoints(): Promise<void> {
    try {
      ...
      const tokenResponse = await fetch('https://httpbin.org/anything/oauth/token', { method: 'POST' })
