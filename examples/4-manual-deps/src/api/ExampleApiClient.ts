...
export class ExampleApiClient extends BaseApiClient {
  ...
  async queryCustomers(params: {
    ...
    try {
      ...
      const filteredCustomers = params.search 
        ? customers.filter(c => c.name.toLowerCase().includes(params.search!.toLowerCase()) ||
  ...
  async queryOrders(params: {
    ...
    try {
      ...
      const orders: Order[] = posts.map((post: any) => ({
        ...
        shippingMethod: ['standard', 'expedited', 'overnight'][post.id % 3] as Order['shippingMethod'],
      ...
      const filteredOrders = params.search 
        ? orders.filter(o => o.id.includes(params.search!) ||
  ...
  async queryProducts(params: {
    ...
    const response = await this.makeRequest<{ items: Product[]; totalCount: number; pagination: any }>(
      'GET',
  ...
  async queryAnalytics(params: {
    ...
    if (params.filters?.dateRange?.startDate) queryParams.set('startDate', params.filters.dateRange.startDate)
    if (params.filters?.dateRange?.endDate) queryParams.set('endDate', params.filters.dateRange.endDate)
