...
// Import tools and dependencies directly (no plugin in manual-deps)
import { ExampleTools } from '../../src/tools/ExampleTools.ts';
 ...
 */
describe('ExampleTools - Manual Registration', () => {
  ...
  beforeEach(() => {
    ...
    // Create ExampleTools instance directly (no plugin in manual-deps)
    ...
    // Get tool registrations directly from ExampleTools
    toolRegistrations = exampleTools.getTools();

    // Verify tools were created successfully
    assertExists(exampleTools, 'ExampleTools should be created');
    assertExists(toolRegistrations, 'ExampleTools should have tool registrations');
  });

  /**
   * MANUAL TOOL REGISTRATION TESTING
   *
   * Tests that verify OAuth-aware tools are properly registered manually.
   */
  describe('Manual Tool Registration with OAuth Tools', () => {
    it('should have OAuth-authenticated tools from manual registration', () => {
      ...
      expectedTools.forEach((toolName) => {
        ...
        assertExists(tool, `Tool '${toolName}' should exist in tool registrations`);
    ...
    it('should handle all OAuth-authenticated tool scenarios', async () => {
      ...
      for (const scenario of testParams) {
        ...
        if (scenario.expectedAuthRequired && scenario.params.userId) {
          ...
          assert(
            mockOAuth.hasValidToken(scenario.params.userId),
...
/**
 * EDUCATIONAL SUMMARY - OAUTH TOOL TESTING (MANUAL REGISTRATION)
 * =============================================================
 *
 * This comprehensive test file demonstrates OAuth-aware tool testing with manual registration:
 *
 ...
 *
 * 3. 🛠️ MANUAL TOOL REGISTRATION TESTING:
 *    - Direct tool instance creation and testing
 *    - Tool registration array verification
 *    - Business logic validation without plugin layer
 ...
 *
 * KEY MANUAL REGISTRATION TESTING BENEFITS:
 * ========================================
 *
 * - **Direct Testing**: Tests tools directly without plugin abstraction
 * - **Simplified Setup**: No plugin creation required for testing
 ...
 * OAuth-authenticated MCP tools with manual dependency registration.
 */
