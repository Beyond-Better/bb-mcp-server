...
describe("ExampleQueryWorkflow - OAuth Integration", () => {
  ...
  describe("OAuth-Authenticated Execution", () => {
    it("should execute customer query with OAuth authentication", async () => {
      ...
      const customerCall = apiCalls.find((call) =>
        call.method === "queryCustomers"
      );
    ...
    it("should execute products query with category filtering", async () => {
      ...
      const apiCalls = mockApiClient.getCallLog();
      const productCall = apiCalls.find((call) =>
        call.method === "queryProducts"
      );
    ...
    it("should execute analytics query with date range", async () => {
      ...
      const apiCalls = mockApiClient.getCallLog();
      const analyticsCall = apiCalls.find((call) =>
        call.method === "queryAnalytics"
      );
...
/**
 * EDUCATIONAL SUMMARY - OAUTH WORKFLOW TESTING (MANUAL REGISTRATION)
 * ==================================================================
 *
 * This comprehensive test file demonstrates OAuth-aware workflow testing with manual registration:
 *
 ...
 *
 * 7. 🛠️ MANUAL WORKFLOW REGISTRATION:
 *    - Direct workflow instance creation and testing
 *    - Workflow testing without plugin abstraction
 *    - Simplified dependency injection for testing
 *    - Direct workflow method invocation
 *
 ...
 *
 * KEY MANUAL WORKFLOW TESTING BENEFITS:
 * ===================================
 *
 * - **Direct Testing**: Tests workflows directly without plugin layer
 ...
 * OAuth-authenticated workflows with manual dependency registration.
 */
