...
export class ExampleQueryWorkflow extends WorkflowBase {
  ...
  private async performDryRun(
    ...
    //console.log('🔍 QUERY DRY RUN START:', {
    //  queryType: params.queryType,
    //  pagination: params.pagination,
    //  paginationPage: params.pagination?.page,
    //  paginationLimit: params.pagination?.limit
    //});
  ...
  private async formatResults(
    ...
    // console.log('🎨 FORMAT RESULTS INPUT:', {
    //   format,
    //   includeMetadata,
    //   resultKeys: Object.keys(results),
    //   hasQuery: 'query' in results,
    //   hasMetadata: 'metadata' in results
    // });
    ...
    switch (format) {
      ...
      case 'summary':
        return {
          ...
          summary: this.generateSummary(results, queryType),
        ...
        const detailedResult = {
          ...
          pagination: results.pagination || {
            ...
            totalPages: Math.ceil(
              (results.totalCount || 0) / (results.limit || 20),
            ),
            hasNext: (results.page || 1) * (results.limit || 20) <
              (results.totalCount || 0),
            hasPrevious: (results.page || 1) > 1,
          },
          summary: this.generateSummary(results, queryType),
          ...(includeMetadata &&
            { metadata: results.metadata || { processed: true } }),
        };
        // console.log('🔍 DETAILED FORMAT RESULT:', {
        //   hasQuery: 'query' in detailedResult,
        //   hasTotalCount: 'totalCount' in detailedResult,
        //   hasItems: 'items' in detailedResult,
        //   hasPagination: 'pagination' in detailedResult,
        //   hasSummary: 'summary' in detailedResult,
        //   hasMetadata: 'metadata' in detailedResult,
        //   includeMetadata,
        //   resultKeys: Object.keys(detailedResult)
        // });
  ...
  private generateSummary(
    ...
    return {
      ...
      ...this.generateTypeSummary(items, queryType || results.queryType),
    };
  }
   ...
   * Validate query parameters
   */
  private async validateQueryParameters(
    params: z.infer<typeof this.parameterSchema>,
  ): Promise<any> {
