...
describe('Plugin OAuth Integration', () => {
  ...
  describe('End-to-End OAuth Authentication', () => {
    it('should complete full OAuth flow across tools and workflows', async () => {
      ...
      assert(apiCalls.every(call => call.userId === userId), 'All calls should use same user context');
    });
    ...
    it('should handle OAuth token refresh during long operations', async () => {
      ...
      assert(tokenInfo.access_token !== 'short_lived_token', `Token should have been refreshed. Got: ${tokenInfo.access_token}`);
    });
  ...
  describe('Comprehensive OAuth Flow Testing', () => {
    ...
    it('should handle OAuth errors consistently across components', async () => {
      ...
      const tools = plugin.tools.filter((t: any) => 
        t.name !== 'get_api_info_example' // This tool doesn't require auth
      );
      
      for (const tool of tools) {
        const result = await tool.handler({
          userId,
          ...(tool.name === 'query_customers_example' ? { search: 'test' } :
             ...
             tool.name === 'get_order_status_example' ? { orderId: 'test' } : {})
        });
        
        assertEquals(result.isError, true, `Tool ${tool.name} should fail with OAuth error`);
        assert(result.content[0].text.includes('OAuth authentication failed'),
      ...
      for (const workflow of workflows) {
        const result = await workflow.executeWithValidation({
          userId,
          ...(workflow.name === 'example_query' ? { queryType: 'customers' } :
             workflow.name === 'example_operation' ? {
               operationType: 'create_customer_with_order',
               operationData: {
                 type: 'create_customer_with_order',
                 customer: { name: 'test', email: 'test@test.com', address: { street: 'test', city: 'test', state: 'TS', zipCode: '12345' } },
                 order: { items: [{ productId: 'test', quantity: 1, unitPrice: 100 }] }
               }
             } : {})
        }, createAuthenticatedWorkflowContext());
        
        assertEquals(result.success, false, `Workflow ${workflow.name} should fail with OAuth error. Got result: ${JSON.stringify(result, null, 2)}`);
