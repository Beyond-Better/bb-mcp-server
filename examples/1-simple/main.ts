  ...
  try {
    ...
    const appServer = await AppServer.create({
      ...
      serverConfig: {
        ...
        description: 'Demonstrates basic plugin tools with minimal setup using bb-mcp-server library',
      },
      
      // 🎯 Optional: Plugin discovery configuration
      // (These are the defaults, shown for educational purposes)
      pluginConfig: {
        discoveryPaths: ['./src/plugins'],
        autoload: true,
        // allowedPlugins: ['SimplePlugin'], // Uncomment to restrict plugins
      },
    });
    ...
    console.log('📝 Available tools: current_datetime, get_system_info, validate_json');
