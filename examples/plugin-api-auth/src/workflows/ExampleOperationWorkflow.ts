...
export class ExampleOperationWorkflow extends WorkflowBase {
  ...
  readonly description = '🛠️ Execute complex ExampleCorp business operations with multi-step processing and rollback capabilities'
  readonly category = 'operation' as const
  override readonly tags = ['operation', 'business', 'examplecorp', 'multi-step']
  ...
  readonly parameterSchema = z.object({
    ...
    operationType: z.enum(['create_customer_with_order', 'bulk_update_inventory', 'process_refund', 'migrate_data']).describe('Type of operation to execute'),
    ...
    operationData: z.discriminatedUnion('type', [
      ...
      z.object({
        ...
        dataTypes: z.array(z.enum(['customers', 'orders', 'products', 'inventory'])).min(1),
    ...
    executionOptions: z.object({
      timeout: z.number().int().min(1).max(3600).optional().default(300).describe('Operation timeout in seconds'),
      retryAttempts: z.number().int().min(0).max(5).optional().default(2).describe('Number of retry attempts on failure'),
      rollbackOnFailure: z.boolean().optional().default(true).describe('Whether to rollback changes on failure'),
      continueOnPartialFailure: z.boolean().optional().default(false).describe('Continue processing if some items fail'),
  ...
  async execute(params: z.infer<typeof this.parameterSchema>): Promise<WorkflowResult> {
    ...
    try {
      ...
      return {
        ...
        completed_steps: operationResult.success ? [{
        ...
        failed_steps: operationResult.success ? [] : [{
          operation: params.operationType,
      ...
      this.logger.error('ExampleCorp operation workflow failed', error instanceof Error ? error : new Error(String(error)), {
        workflowName: this.name,
        operationType: params.operationType,
        operationId,
        executionTime,
      ...
      if (params.notifications?.notifyOnFailure) {
        await this.sendNotification(params, { success: false, error }, 'failure')
      }
   ...
   */
  private async performDryRun(params: z.infer<typeof this.parameterSchema>, operationId: string): Promise<WorkflowResult> {
  ...
  private async executeOperation(
    ...
    operationId: string
  ): Promise<{ success: boolean; data?: any; steps?: any[]; rollbackInfo?: any; error?: any }> {
    const steps: any[] = []
    ...
    try {
      ...
      switch (params.operationType) {
        case 'create_customer_with_order':
          return await this.executeCreateCustomerWithOrder(params, operationId, steps, rollbackActions)
          
        case 'bulk_update_inventory':
          return await this.executeBulkInventoryUpdate(params, operationId, steps, rollbackActions)
          
        case 'process_refund':
          return await this.executeProcessRefund(params, operationId, steps, rollbackActions)
          
        case 'migrate_data':
          return await this.executeDataMigration(params, operationId, steps, rollbackActions)
      ...
      if (params.executionOptions?.rollbackOnFailure && rollbackActions.length > 0) {
      ...
      return {
        ...
        steps,
        rollbackInfo: {
          rollbackPerformed: params.executionOptions?.rollbackOnFailure && rollbackActions.length > 0,
          rollbackActions: rollbackActions.length,
        },
      }
    }
  ...
  private async executeCreateCustomerWithOrder(
    ...
    const operationData = params.operationData as Extract<typeof params.operationData, { type: 'create_customer_with_order' }>
    ...
    const customer = await this.apiClient.createCustomer(customerData, params.userId)
    steps.push({ step: 'create_customer', result: customer, timestamp: new Date().toISOString() })
    rollbackActions.push({ action: 'delete_customer', customerId: customer.id })
    
    // Step 2: Create order for customer
    this.logger.debug('Creating order for customer', { operationId, customerId: customer.id })
    const orderData: any = {
    ...
    steps.push({ step: 'create_order', result: order, timestamp: new Date().toISOString() })
  ...
  private async executeBulkInventoryUpdate(
    ...
    const operationData = params.operationData as Extract<typeof params.operationData, { type: 'bulk_update_inventory' }>
  ...
  private async executeProcessRefund(
    ...
    const operationData = params.operationData as Extract<typeof params.operationData, { type: 'process_refund' }>
    ...
    const refund = await this.apiClient.processRefund(refundData, params.userId)
  ...
  private async executeDataMigration(
    ...
    const operationData = params.operationData as Extract<typeof params.operationData, { type: 'migrate_data' }>
    
    if (operationData.validateOnly) {
      // Validation-only migration
      const validation = await this.apiClient.validateDataMigration(operationData, params.userId)
      steps.push({
      ...
      const migration = await this.apiClient.executeDataMigration(operationData, params.userId)
      steps.push({
   ...
   */
  private async performRollback(rollbackActions: any[], operationId: string): Promise<void> {
    for (const action of rollbackActions.reverse()) { // Reverse order for rollback
      try {
        ...
        this.logger.error('Rollback action failed', error instanceof Error ? error : new Error(String(error)), {
          operationId,
   ...
   */
  private async executeRollbackAction(action: any, operationId: string): Promise<void> {
   ...
   */
  private async buildExecutionPlan(params: z.infer<typeof this.parameterSchema>): Promise<any> {
    const baseSteps = {
   ...
   * Validate operation permissions
   */
  private async validatePermissions(params: z.infer<typeof this.parameterSchema>): Promise<any> {
   ...
   */
  private estimateExecutionTime(params: z.infer<typeof this.parameterSchema>): string {
    const estimates = {
   ...
   */
  private calculateResourceRequirements(params: z.infer<typeof this.parameterSchema>): any {
    return {
   ...
   */
  private estimateApiCalls(params: z.infer<typeof this.parameterSchema>): number {
    switch (params.operationType) {
      case 'create_customer_with_order':
        return 2 // create customer + create order
      case 'bulk_update_inventory':
        const updates = params.operationData.type === 'bulk_update_inventory' ? params.operationData.updates : []
        return Math.ceil(updates.length / 100) + 1 // batch API calls
      case 'process_refund':
        return 2 // validate order + process refund
      case 'migrate_data':
        const dataTypes = params.operationData.type === 'migrate_data' ? params.operationData.dataTypes : []
